...
BEGIN
  ...
  FOR r IN
    ...
    WHERE
      s.status = 'dipinjam' AND s.tanggal_kembali::date <= CURRENT_DATE -- Cast to date for comparison
  LOOP
    RAISE NOTICE 'Memproses sirkulasi ID: %, NIS: %, Tanggal Kembali: %, Status: %', r.id_sirkulasi, r.id_nis, r.tanggal_kembali, r.status;

    -- Calculate days late by casting tanggal_kembali to date
    v_days_late := (CURRENT_DATE - r.tanggal_kembali::date);
