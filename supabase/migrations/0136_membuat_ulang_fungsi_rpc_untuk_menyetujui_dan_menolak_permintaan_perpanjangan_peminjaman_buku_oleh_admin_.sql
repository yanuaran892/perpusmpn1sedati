-- Recreate approve_extension_request_admin function
...
BEGIN
  ...
  -- Update sirkulasi status to 'dipinjam', increment extension count, and set new return date
  UPDATE public.sirkulasi
  SET
    status = 'dipinjam',
    tanggal_kembali = v_sirkulasi_record.tanggal_kembali_request, -- Set new return date from request
    jumlah_perpanjangan = v_sirkulasi_record.jumlah_perpanjangan + 1,
    tanggal_kembali_request = NULL, -- Clear the request date
...
-- Recreate reject_extension_request_admin function
