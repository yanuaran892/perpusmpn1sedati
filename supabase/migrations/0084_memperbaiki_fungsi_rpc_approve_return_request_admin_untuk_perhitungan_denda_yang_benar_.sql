...
BEGIN
  ...
  -- Ensure both timestamps are cast to DATE before subtraction
  v_days_late := (v_sirkulasi_record.tanggal_dikembalikan::date - v_sirkulasi_record.tanggal_kembali::date);
  ...
  RAISE NOTICE 'Sirkulasi ID: %, Tanggal Kembali (Estimasi): %, Tanggal Dikembalikan (Aktual): %',
               p_sirkulasi_id, v_sirkulasi_record.tanggal_kembali, v_sirkulasi_record.tanggal_dikembalikan::date;
