CREATE OR REPLACE FUNCTION public.update_siswa_password(
  p_id_nis text,
  p_old_password text,
  p_new_password text
)
...
BEGIN
  ...
  IF v_current_hashed_password IS NULL OR v_current_hashed_password <> crypt(p_old_password, v_current_hashed_password) THEN
