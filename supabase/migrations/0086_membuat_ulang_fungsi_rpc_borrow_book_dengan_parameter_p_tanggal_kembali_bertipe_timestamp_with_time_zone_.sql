CREATE OR REPLACE FUNCTION public.borrow_book(p_id_nis text, p_id_buku integer, p_tanggal_pinjam timestamp with time zone, p_tanggal_kembali timestamp with time zone)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.sirkulasi (
    ...
    p_id_buku,
    p_tanggal_pinjam,
    p_tanggal_kembali, -- Now uses TIMESTAMP WITH TIME ZONE directly
    'pending',
    0
  );
