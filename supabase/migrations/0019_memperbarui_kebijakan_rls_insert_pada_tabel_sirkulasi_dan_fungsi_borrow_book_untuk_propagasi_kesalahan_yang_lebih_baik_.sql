...
EXCEPTION
  WHEN OTHERS THEN
    RAISE EXCEPTION 'Error in borrow_book function: %', SQLERRM; -- Changed RAISE WARNING to RAISE EXCEPTION
    RETURN FALSE; -- This return will not be reached if RAISE EXCEPTION is called
END;
$function$;

-- The approve_borrow_request and reject_borrow_request functions remain unchanged from the previous step.
-- They are included here for completeness of the SQL migration, but their content is identical.
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE EXCEPTION 'Error in approve_borrow_request function: %', SQLERRM;
    RETURN FALSE;
END;
$function$;
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE EXCEPTION 'Error in reject_borrow_request function: %', SQLERRM;
    RETURN FALSE;
END;
$function$;