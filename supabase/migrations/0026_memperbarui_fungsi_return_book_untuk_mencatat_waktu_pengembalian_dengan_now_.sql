CREATE OR REPLACE FUNCTION public.return_book(p_sirkulasi_id integer, p_id_buku integer, p_id_nis text)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
...
AS $function$
DECLARE
  v_tanggal_kembali DATE; -- Keep as DATE for comparison with CURRENT_DATE
  v_days_late INTEGER := 0;
  v_calculated_denda NUMERIC := 0;
  v_current_sedang_pinjam INTEGER;
  v_current_total_denda NUMERIC;
BEGIN
  -- 1. Get expected return date (still a DATE for comparison)
  ...
  SET
    status = 'dikembalikan',
    tanggal_dikembalikan = NOW(), -- Changed to NOW() to include time
  ...
  UPDATE public.siswa
  SET
    sedang_pinjam = GREATEST(0, v_current_sedang_pinjam - 1), -- Memastikan sedang_pinjam tidak kurang dari 0
